...
- **공유 옵션 세트(선택지 목록)**: 사이드바 "옵션 세트" → 변수와 **같은 이름**의 세트(예: `의상`=선택지 30개)를 만들면, 미리보기에서 그 변수(`{{의상}}`)가 **드롭다운(골라 쓰기)**이 된다. 자유입력 변수는 그대로 텍스트, "✎ 직접 입력"으로 임시 자유입력 전환 가능. 세트는 앱 전체 공유(여러 프롬프트가 같은 목록 사용) + 폴더/JSON 동기화에 포함 + tombstone 병합. 용도: 경우의 수가 많고 수정부분이 명확한 프롬프트(예: 이미지 생성 프롬프트의 `{{의상}}`·`{{상황}}`)에서 본문·변수는 고정하고 선택지만 골라 최종본 생성.
...
- ② 추가: **공유 옵션 세트(선택지 목록)** — 변수 이름과 같은 세트를 만들면 미리보기에서 드롭다운. 앱 전체 공유·폴더/JSON 동기화·tombstone 병합·"직접 입력" 폴백. DB meta에 optionSets/optionSetsDeleted 저장. 로컬 검증: 세트 생성·영속, 매칭 변수 드롭다운·치환, 미매칭 변수 텍스트유지, 직접입력↔목록 토글, 콘솔 에러 0.
